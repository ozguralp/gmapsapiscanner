...
def scan_gmaps(apikey):
	...
	response = requests.get(url, verify=False)
	if response.text.find("error_message") < 0:
		print "API key is \033[1;31;40m vulnerable \033[0m for Query Autocomplete-Places API! Here is the PoC link which can be used directly via browser:"
		print url
	...
	if response.status_code == 200:
		...
		for lines in response.iter_lines():
			if(("TITLE") in lines):
				print "Reason: "+lines.split("TITLE")[1].split("<")[0].replace(">","")
	...
	jsapi = raw_input("Do you want to conduct tests for Javascript API? (Will need manual confirmation + file creation) (Y/N)")
	if jsapi == "Y" or jsapi == "y":
		...
		result = raw_input("Press enter again for deletion of jsapi_test.html file automatically after manual confirmation is conducted.")
